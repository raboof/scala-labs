...
class RecursionPatternMatchingExerciseTest extends Specification {
  ...
  "RecursionPatternMatchingExercise" should {
    ...
    "group equal members" in {
      skipped("Fix me")
      List(List(1, 1, 1, 1), List(5, 5), List(4, 4)) === RecursionPatternMatchingExercise.groupEquals(List(1, 1, 1, 5, 4, 4, 5, 1))
    }
    "remove consecutive duplicates" in {
      skipped("Fix me")
      List(1, 5, 8, 4, 9) === RecursionPatternMatchingExercise.compress(List(1, 1, 1, 1, 5, 8, 8, 4, 4, 4, 9, 9))
    }
    "define amount equal members" in {
      skipped("Fix me")
      List((4, 'x), (2, 'y), (2, 'z)) === RecursionPatternMatchingExercise.amountEqualMembers(List('x, 'x, 'x, 'y, 'z, 'z, 'y, 'x))
      List((4, "Cow"), (2, "Boy"), (1, "Hut")) === RecursionPatternMatchingExercise.amountEqualMembers(List("Cow", "Cow", "Boy", "Cow", "Boy", "Hut", "Cow"))
    }
    "zip multiple" in {
      skipped("Fix me")
      List(List(1, 'A, 'a), List(2, 'B, 'b), List(3, 'C, 'c)) === RecursionPatternMatchingExercise.zipMultiple(List(List(1, 2, 3), List('A, 'B, 'C), List('a, 'b, 'c)))
    }
    "zip multiple with different size" in {
      skipped("Fix me")
      List(List(1, 'A, 'a)) === RecursionPatternMatchingExercise.zipMultipleWithDifferentSize(List(List(1, 2), List('A, 'B, 'C), List('a)))
    }
  }
}
